...
- **Langfuse tracing (LOCAL DEV ONLY, issue #18 Phase 1)** — `make langfuse-up` starts a self-hosted Langfuse v4 stack (`docker compose --profile observability`), and `LANGFUSE_ENABLED=true` attaches a `langfuse.langchain.CallbackHandler` to every graph invocation via `integrations.get_callbacks()` (wired in `cli.py` and `api/services/observable_agent.py`). Zero GCP footprint by construction: the flag is never set in `build-deploy.yml`/`deploy.sh`, and the SDK (plus the `langchain` meta-package it imports) lives only in `requirements-dev.txt` with lazy imports, so the prod image can't even fail on import — `tests/unit/test_langfuse_integration.py` asserts both. UI at `http://localhost:3000` (login `dev@example.com` / `localdev123`); API keys are pre-provisioned via `LANGFUSE_INIT_*` in `docker-compose.yml` and match the `config.py` defaults, so the flag is the only switch. Note: there is no pre-commit hook in `.git/hooks` despite the note below — run `make smoke-local-quick` by hand.
  ...
  - **Eval/annotation workflows (issue #56 Phase B)** — `integrations/langfuse_eval.py` links the app's existing ESCI ground-truth judgments (`agent_state.judgments`, from `VectorStore.lookup_judgments`) into Langfuse's own primitives, not ad-hoc score calls: `sync_dataset_item` idempotently upserts each query's judgments as a **Dataset** item (`esci-ground-truth`, keyed by lowercased query); `record_citation_eval` scores `eval_citation_precision` (fraction of cited products with a positive ESCI relevance label) opportunistically on every live traced request with ground truth for its query (`agent_node`'s citation-building return sites in `pipeline_nodes.py`), and pushes it to the `low-confidence-citations` **Annotation Queue** (`queue_for_human_review`, auto-creates its backing Score Config on first use) when precision is below `LOW_PRECISION_THRESHOLD`. `make langfuse-eval` (`scripts/run_langfuse_eval.py`) runs a proper **Experiment** — Langfuse's own `client.run_experiment()` SDK method (task + evaluator functions against dataset items, auto-traced, auto-linked to a Dataset Run) — populating the Datasets > Runs UI with a real, comparable-over-time run, not just scattered live scores. (Registering `eval_citation_precision` as a formal Langfuse **Evaluator**/`/evals` object was investigated but deliberately not done: self-hosted Code Evaluators only execute locally via the `insecure-local` dispatcher, which supports TypeScript only, not Python — `run_experiment()`'s Python evaluators are the idiomatic, fully-local-Python alternative Langfuse itself documents for this exact task+evaluator pattern.)
