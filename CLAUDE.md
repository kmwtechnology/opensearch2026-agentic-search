...
| Deploy mechanism | **Production deployment is fully automated:** push to `main` → `build-deploy.yml`'s `deploy-cloud-run` job, gated on `build-docker` + `unit-tests` + `integration-tests` + `lint-backend` + `frontend-tests` + `shellcheck` all passing. A trailing `notify` job posts one consolidated summary. **Manual deployment:** `langchain_agent/scripts/deploy.sh` and `gcp-init.sh` exist for local/manual GCP setup (documented in `docs/operations/gcp-quickstart.md`). |
