...
- **Auth** — same-origin checking (`origin_auth.py`) is the sole auth layer: allow-list of localhost ports + Cloud Run `*.run.app`; disallowed Origin always 403s. The shared-password session login gate (`REQUIRE_LOGIN`/`LOGIN_PASSWORD`, `/api/auth/*`, `ahs_session` cookie, WS close 4401) was removed entirely (issue #135) — it was already optional and off by default, so nothing observable changed. **Admin token** (`api/middleware/admin_auth.py`, `ADMIN_TOKEN` env var, `X-Admin-Token` header, constant-time comparison via `hmac.compare_digest`) is preserved as a standalone utility for future automation, but is **not** currently wired into any route — `/api/admin/*` routes use same-origin checking only.
  - **Do NOT wire new routes through `verify_api_key`** — it doesn't exist anywhere in `api/middleware/` (there is no `auth.py` module at all; it held only `AuthConfigurationError` and was deleted with the login gate, issue #135). Importing it raises `ImportError`. Use `verify_same_origin` (and `verify_admin_token` from `api/middleware/admin_auth.py` if a route specifically needs automation-token access).
