...
| CI / deploy watch (`/workflow-deploy`) | Hyrule's k8s `build`/`deploy` workflow names | This repo's actual workflows: `.github/workflows/build-deploy.yml` (PR + main CI/deploy) and `.github/workflows/reindex.yml` (manual Lucille ETL trigger) — see **CI/CD** below |
...
- `langchain_agent/` — main application
  ...
  - `setup.py`, `benchmark_search.py`, `checkpoint_maintenance.py`, `checkpoint_optimizer.py`
  - `api/` — FastAPI backend
...
- **Attribute normalization** — integrated into the Lucille ETL pipeline via `AttributeNormalizerStage` (custom Java stage). `product_color_primary`, `product_color_secondary`, `product_brand_normalized` are keyword fields set during ingest, not as a post-processing step. Uses deterministic rules: 16 canonical colors (e.g., "grey" → "gray"), synonym expansion, compound extraction ("Black & Purple" → primary: "black", secondary: "purple"), brand case-folding. Improves recall: "blue" now matches "Navy", "Cyan", "Teal", etc. Raw fields preserved, no data loss. Reproducible and auditable (rules-only, no AI). See **Lucille Custom Stages** below.
...
The custom `lucille-esci` Maven module (configs + mappings) lives in-repo. **`lucille_ingest.sh` defaults to running Lucille via Docker** (`LUCILLE_USE_DOCKER=true`, the default for local dev, `reindex.yml`, and `gcp-init.sh` alike) — `docker/lucille/Dockerfile` resolves `lucille-core`/`lucille-parquet` from Maven Central using `lucille-esci/pom.xml` as the dependency manifest; no local Java/Maven/Lucille checkout needed anywhere. The container targets the local `opensearch` compose service when `OPENSEARCH_HOST=localhost`, or a remote hosted OpenSearch directly otherwise (CI, a GCP workstation). `LUCILLE_USE_DOCKER=false` is a fallback to the native path for environments without Docker: Step 1 builds `lucille-bom` + `lucille-parquet` from an external Lucille checkout (default `~/github/kmwtechnology/lucille`, override via `LUCILLE_DIR`) into `~/.m2` on first run only. The pinned version has a single source of truth — `LUCILLE_VERSION` in `langchain_agent/.env.example` — read by `lucille_ingest.sh`, `lucille-esci/pom.xml` (via `${env.LUCILLE_VERSION}`), `docker-compose.yml`'s `lucille` service build arg, and `reindex.yml`; bump it there only (see `lucille-esci/README.md` "Versioning").
...
2. `products.conf` (Lucille) — sets `chunk_text`, `collection_id=esci_products`, dual-mapped brand/color fields; **also copies `product_title → title_suggest` and `product_brand → brand_suggest`** (edge-ngram fields required by `/api/suggest`; missing = typeahead returns 0 results); runs `normalizeAttributes` stage (custom Java stage, see **Lucille Custom Stages** below)
...
### Lucille Custom Stages

The ingest pipeline includes custom Java stages in `langchain_agent/lucille-esci/src/main/java/com/kmwllc/esci/`:

**`AttributeNormalizerStage`** — normalizes product colors and brands during ingest (runs as the `normalizeAttributes` stage in `products.conf`). Reduces ingest from two-pass (Lucille → OpenSearch → Python bulk-update) to single-pass (Lucille → OpenSearch). Configuration: loads `color_mappings.json` at startup for 16 canonical colors + brand generics. Outputs: `product_color_primary`, `product_color_secondary`, `product_brand_normalized` as keyword fields for filter precision.

**Pattern for adding new stages:** (1) Extend `Stage`, implement `start()` (config parsing, one-time setup) and `processDocument(Document doc)` (per-document transformation); (2) Add to `pom.xml` dependencies as needed; (3) Reference in `products.conf` or `judgments.conf` as a stage in the pipeline; (4) Include in `docker/lucille/Dockerfile` multi-stage build (Maven compile + resolve dependencies).

Docker build: `docker/lucille/Dockerfile` compiles custom stages (`mvn package`) and resolves all runtime dependencies from Maven Central via `lucille-esci/pom.xml`, then layers compiled jars into a minimal JRE image. No local Java/Maven checkout required.
...
- `gcp-init.sh` — first-time GCP setup: Cloud SQL (PostgreSQL 16), schema + checkpoints table, product + judgment ingestion **via Lucille ETL** (runs on a workstation with Docker — the Lucille container targets the remote hosted OpenSearch directly, no local compose stack needed; `LUCILLE_USE_DOCKER=false` falls back to a native Java/Maven/Lucille checkout; reads static parquets from `data/`), API validation. It exports `OPENSEARCH_HOST`/`PORT`/`USE_SSL` for the hosted instance; `lucille_ingest.sh` loads `.env` with **non-override** semantics so those exports win (otherwise `.env`'s `localhost` would silently capture the ingest). **There is no in-container ingest baked into the deployed app** — the slim Cloud Run image has no Java/Docker-in-Docker; re-indexing is triggered via `reindex.yml` (Lucille runs via Docker on the Actions runner, which has Docker preinstalled) or manually via `lucille_ingest.sh` with the hosted OpenSearch vars.
- `gcp-teardown.sh` — removes Cloud Run, Cloud SQL + backups, OpenSearch, Artifact Registry, Secrets.

## CI/CD — GitHub Actions

- `.github/workflows/build-deploy.yml` — every PR/push: backend unit + integration (with PostgreSQL + OpenSearch), lint (flake8/black/isort/mypy), Playwright e2e, coverage. On merges to `main`: Docker build (cached) → push to `us-central1-docker.pkg.dev/<PROJECT_ID>/agentic-hybrid-search/agentic-hybrid-search:latest` → Cloud Run blue-green deploy → smoke tests on `/health` → 100% traffic. **Docker layer note:** `torch` CPU-only wheel is installed as a separate layer before `pip install -r requirements.txt` — this keeps the pip layer ~750MB instead of ~3GB (Cloud Run is CPU-only; GPU CUDA libraries in the default PyPI wheel are dead weight). LFS objects (`data/*.parquet`) are cached between builds via `actions/cache` keyed on file content hash. **Cross-encoder baked into image (issue #78):** the reranker model (`cross-encoder/ms-marco-MiniLM-L-12-v2`, ~128MB, `config.CROSS_ENCODER_MODEL`) is pre-downloaded at build time into `HF_HOME=/opt/hf-cache`, and runtime sets `HF_HUB_OFFLINE=1` + `TRANSFORMERS_OFFLINE=1`. **Do NOT remove these** — without the baked model the image fetches it from HuggingFace Hub *unauthenticated at every cold start*, which trips HF's anonymous per-IP rate limit during a deploy burst and hangs agent init indefinitely (full outage; took down prod 2026-06-13, see `memory/cross_encoder_hf_runtime_fetch_outage.md`). Keep the Dockerfile model id in sync with `config.CROSS_ENCODER_MODEL`.
- `.github/workflows/reindex.yml` — manual-dispatch workflow for re-ingesting ESCI data via Lucille ETL. Runs on the Actions runner (which has Docker preinstalled), builds the Lucille container, and targets the remote hosted OpenSearch via WIF-authenticated Secret Manager credentials.
- Auth: Workload Identity Federation (no long-lived keys). GitHub secrets: `WIF_PROVIDER`, `WIF_SERVICE_ACCOUNT`. Service account `github-actions@gen-lang-client-0250737934.iam.gserviceaccount.com` has roles `artifactregistry.writer`, `run.developer`, `iam.serviceAccountUser`. The WIF provider (`projects/375500751528/locations/global/workloadIdentityPools/github-actions/providers/github`) trusts the whole `kmwtechnology` GitHub org via `attribute_condition: repository_owner == 'kmwtechnology'`, but the service account's `roles/iam.workloadIdentityUser` binding is scoped **per-repo** via a `principalSet` on `attribute.repository` — a new repo (e.g. one duplicated from another, as this one was) needs its own principalSet added to that binding before its Actions runs can deploy, even though the provider itself already trusts it. Check with `gcloud iam service-accounts get-iam-policy github-actions@gen-lang-client-0250737934.iam.gserviceaccount.com --project=gen-lang-client-0250737934`; add with `gcloud iam service-accounts add-iam-policy-binding ... --member="principalSet://iam.googleapis.com/projects/375500751528/locations/global/workloadIdentityPools/github-actions/attribute.repository/<owner>/<repo>"`. Symptom when missing: `Build Docker Image` fails with `google-github-actions/auth failed: ... must specify exactly one of "workload_identity_provider" or "credentials_json"` even with both GitHub secrets correctly set — a misleading error that looks like a secrets-config problem, not an IAM-scope problem.
...
- **Unit tests** (~730+ total: 600+ Python backend + 120+ frontend) — no external deps; `PYTHONPATH=. pytest tests/unit/` (~3s).
