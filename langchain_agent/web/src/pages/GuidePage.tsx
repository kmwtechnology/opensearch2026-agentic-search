      ...
      content: (
        <div className="space-y-4">
          <div className="space-y-3">
            ...
            <p className="text-[1.375rem] text-gray-600 mt-1">Runs a quick smoke test to verify all components are working (~13s)</p>
          </div>
        </div>
      ),
    },
        ...
        <div className="space-y-4">
          <div className="space-y-3">
            <div className="border-l-4 border-green-500 pl-4">
              ...
              <p className="text-[1.375rem] text-gray-600">Combines semantic (vector) and lexical (BM25) search with automatic tuning based on query type</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Example: "Sony headphones" uses 25% semantic, 75% lexical for brand specificity</p>
            </div>

            <div className="border-l-4 border-purple-500 pl-4">
              <h4 className="font-semibold text-gray-900">Intent Classification</h4>
              <p className="text-[1.375rem] text-gray-600">Seven classes: search, comparison, attribute_filter, refinement, follow_up, summary, clarify</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Single structured-output LLM call (no keyword fast-path). Confidence below 0.7 returns `clarify` intent with clarification questions instead of a guess.</p>
            </div>

            <div className="border-l-4 border-orange-500 pl-4">
              <h4 className="font-semibold text-gray-900">Smart Reranking</h4>
              <p className="text-[1.375rem] text-gray-600">Cross-encoder (ms-marco-MiniLM-L-12-v2) relevance scoring on 0.0–1.0 scale to ensure top results are truly relevant</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Default: cross-encoder (~2s for a 40-doc batch, local, measured in production). Optional: Gemini Flash Lite LLM-based reranker (~500ms–1s). Quality gate: retries with adjusted search (alpha ±0.3) if score &lt; 0.5.</p>
            </div>

            <div className="border-l-4 border-pink-500 pl-4">
              <h4 className="font-semibold text-gray-900">Conversation Memory</h4>
              <p className="text-[1.375rem] text-gray-600">Maintains full message history per thread, smart context management for long conversations</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Resume conversations with thread_id; messages stored in PostgreSQL</p>
            </div>

            <div className="border-l-4 border-cyan-500 pl-4">
              <h4 className="font-semibold text-gray-900">Real-time Observability</h4>
              <p className="text-[1.375rem] text-gray-600">Watch the entire pipeline execute in real-time with typed events</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Search → Rerank → Quality Gate → LLM, plus token-by-token streaming</p>
            </div>

            <div className="border-l-4 border-indigo-500 pl-4">
              <h4 className="font-semibold text-gray-900">Search Optimization Toggles</h4>
              <p className="text-[1.375rem] text-gray-600">Flip any of 10 search features on or off per query and watch the pipeline respond live.</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">Hybrid · Fuzzy · Synonyms · Phonetic · Phrase boost · Field boost · Typeahead · Reranker · LLM · LLM Judge. Skipped stages collapse out of the panel automatically.</p>
            </div>

            <div className="border-l-4 border-yellow-500 pl-4">
              <h4 className="font-semibold text-gray-900">Pipeline Quality Summary</h4>
              <p className="text-[1.375rem] text-gray-600">End-of-pipeline scorecard rendered as the last card in the Observability panel.</p>
              <p className="text-[1.25rem] text-[var(--color-stage-ink-soft)] mt-1">When the query is in ESCI: BM25 → Hybrid → Reranked NDCG@10 / MRR / Recall@20 / Precision@10 with latency lift-per-100ms. Otherwise: a self-referential confidence proxy.</p>
            </div>
      ...
      content: (
        <div className="space-y-4">
          <div className="space-y-3">
            ...
            <pre className="bg-[var(--color-stage-bg)] text-[var(--color-stage-ink)] p-3 rounded text-[1.25rem] overflow-x-auto">
              <code>{`POST /api/chat
...
{"type": "AgentCompleteEvent", "response": "...", "citations": [...]}`}</code>
          ...
          <div className="bg-amber-50 border-l-4 border-amber-500 p-3 mt-4">
            <p className="text-[1.375rem] font-semibold text-amber-900">Use WebSocket for:</p>
            <ul className="text-[1.375rem] text-amber-800 mt-1 space-y-1">
          ...
          <div className="bg-blue-50 border-l-4 border-blue-500 p-3 mt-4">
            <p className="text-[1.375rem] font-semibold text-blue-900">Use REST for:</p>
            <ul className="text-[1.375rem] text-blue-800 mt-1 space-y-1">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 text-[1.375rem]">
            <div className="bg-gray-50 p-3 rounded">
              ...
              <p className="text-gray-600 text-[1.25rem] mt-1">Full health status of all dependencies (postgres, google_ai, vector_store)</p>
              <p className="text-[var(--color-stage-ink-soft)] text-[1.25rem] mt-1">Always returns 200, check status field for degraded</p>
            </div>
            <div className="bg-gray-50 p-3 rounded">
              <p className="font-mono text-gray-900">GET /api/health/ready</p>
              <p className="text-gray-600 text-[1.25rem] mt-1">Kubernetes-style readiness probe (200 if ready, 503 if degraded)</p>
            </div>
            <div className="bg-gray-50 p-3 rounded">
              <p className="font-mono text-gray-900">GET /api/config</p>
              <p className="text-gray-600 text-[1.25rem] mt-1">Runtime API URL discovery for frontend</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="bg-blue-50 p-3 rounded">
              ...
              <p className="text-[1.375rem] text-blue-800">"What wireless earbuds have the best noise cancellation?"</p>
            </div>
            <div className="bg-green-50 p-3 rounded">
              <p className="font-semibold text-green-900">Comparison</p>
              <p className="text-[1.375rem] text-green-800">"Compare iPhone 15 Pro vs Samsung Galaxy S24 Ultra"</p>
            </div>
            <div className="bg-purple-50 p-3 rounded">
              <p className="font-semibold text-purple-900">Attribute Filter</p>
              <p className="text-[1.375rem] text-purple-800">"Show me red running shoes size 10"</p>
            </div>
            <div className="bg-orange-50 p-3 rounded">
              <p className="font-semibold text-orange-900">Follow-up</p>
              <p className="text-[1.375rem] text-orange-800">User: "What are good gaming laptops?"<br/>Then: "Which one has the best battery life?"</p>
            </div>
            <div className="bg-pink-50 p-3 rounded">
              <p className="font-semibold text-pink-900">Brand-Specific</p>
              <p className="text-[1.375rem] text-pink-800">"What Sony cameras are available?"</p>
            </div>
          </div>

          <div className="mt-4 bg-amber-50 border-l-4 border-amber-500 p-3">
            <p className="text-[1.375rem] font-semibold text-amber-900">Note on Pricing</p>
            <p className="text-[1.375rem] text-amber-800 mt-1">
      ...
      content: (
        <div className="space-y-4">
          <div className="space-y-2 text-[1.375rem]">
            ...
            <ol className="list-decimal list-inside space-y-1 text-[1.25rem] text-gray-700">
        ...
        <div className="space-y-4">
          <div className="space-y-3 text-[1.375rem]">
            ...
            <div className="border-l-4 border-red-500 pl-3">
              ...
              <p className="text-[var(--color-stage-ink-soft)] text-[1.25rem] mt-1">Fix: <code className="bg-gray-100 px-1">docker compose up -d</code> from repo root</p>
            </div>
            ...
            <div className="border-l-4 border-red-500 pl-3">
              ...
              <p className="text-[var(--color-stage-ink-soft)] text-[1.25rem] mt-1">Fix: Start API with <code className="bg-gray-100 px-1">make dev-api</code></p>
            </div>
            ...
            <div className="border-l-4 border-yellow-500 pl-3">
              ...
              <p className="text-[var(--color-stage-ink-soft)] text-[1.25rem] mt-1">Good: "Sony WH-1000XM5" vs Bad: "good headphones"</p>
            </div>
      ...
      content: (
        <div className="space-y-3 text-[1.375rem]">
          <div className="space-y-2">
            ...
            <ul className="space-y-1 text-gray-600 font-mono text-[1.25rem]">
